...
feature 'Spanish nurse, Participant tasks', metadata: :first do
  ...
  scenario 'Spanish nurse sees translations for confirmation call' do
    ...
    expect(spanish_reschedule_form).to have_alt_date_selections
  ...
  scenario 'Spanish nurse sees translations for non-scheduled tasks' do
    ...
    expect(spanish_lack_of_connectivity_call).to have_helper_text
    ...
    expect(spanish_help_request).to have_helper_text
    ...
    expect(spanish_non_adherence_call).to have_helper_text
  ...
  scenario 'Spanish nurse sees translations for contact supervisor' do
    ...
    expect(pt_501_supervisor_contact).to have_new_supervisor_contact
    ...
    pt_501_supervisor_contact.clear

    expect(pt_501_supervisor_contact).to_not have_new_supervisor_contact
  end
end
...
feature 'Spanish nurse, Participant tasks', metadata: :not_first do
  ...
  scenario 'Spanish nurse confirms final appointment, sees proper alert' do
    pt_517_nurse_tasks.open
    spanish_final_appointment.confirm
    spanish_final_appointment.enter_location
    spanish_final_appointment.enter_notes
    spanish_final_appointment.choose_phone_returned
    spanish_navigation.submit

    expect(spanish_final_appointment).to have_success_alert
  end
end
